...
POP предназначен для получения электронной почты с почтового сервера и загрузки сообщений на устройство клиента. После
загрузки сообщения обычно удаляются с сервера, хотя современные версии протокола могут поддерживать сохранение копий.

### **Принцип работы**

1. **Установка соединения**:
    - Клиент подключается к почтовому серверу через TCP.
    - **Порты**:
        - Порт 110 — стандартный порт для незашифрованного соединения.
        - Порт 995 — для защищенного соединения через SSL/TLS.
2. **Аутентификация**:
    - Клиент отправляет логин и пароль для доступа к почтовому ящику.
3. **Получение почты**:
    - Клиент загружает все сообщения с сервера.
4. **Завершение соединения**:
    - После загрузки соединение разрывается, и дальнейшая работа с письмами происходит локально на клиенте.

### **Особенности POP**

- Подходит для работы с одним устройством, так как удаляет сообщения с сервера.
- Простота реализации и низкая нагрузка на сервер.
- Отсутствие синхронизации между клиентами.
...
IMAP позволяет клиенту работать с письмами, которые остаются на сервере. Поддерживает синхронизацию между устройствами и
доступ к одной почтовой учетной записи с разных клиентов.
...
1. **Установка соединения**:
    ...
    - **Порты**:
        - Порт 143 — стандартный порт для незашифрованного соединения.
        - Порт 993 — для защищенного соединения через SSL/TLS.
2. **Аутентификация**:
    - Клиент предоставляет учетные данные для входа в почтовый ящик.
3. **Доступ к сообщениям**:
    - Сообщения остаются на сервере, и клиент работает с их копиями.
    - Возможности:
        - Просмотр заголовков писем.
        - Загрузка отдельных сообщений.
        - Организация писем в папки.
4. **Поддержка синхронизации**:
    - Изменения, внесенные на одном устройстве, видны на других клиентах.
5. **Завершение соединения**:
    - После завершения сеанса изменения сохраняются на сервере.

### **Особенности IMAP**

- Подходит для работы с несколькими устройствами.
- Сообщения остаются на сервере, обеспечивая резервное копирование.
- Возможность управления папками и просмотра заголовков перед загрузкой письма.

### **Коды ответа IMAP**

IMAP использует трехчастные ответы: тег, статус и текст.

- **Теги**:
    - Уникальные идентификаторы команд.
    - Пример: `A001 LOGIN user password`
- **Статусы**:
    - `OK`: Успех.
    - `NO`: Ошибка, например, отказ в выполнении команды.
    - `BAD`: Синтаксическая ошибка или неверная команда.
- **Примеры ответов**:
    - `A001 OK LOGIN completed`: Успешный вход.
    - `A002 NO Mailbox does not exist`: Ошибка доступа к папке.
    - `A003 BAD Command unknown`: Команда не распознана сервером.

---

## **Сравнение POP и IMAP**

| Характеристика        | POP                   | IMAP                          |
|-----------------------|-----------------------|-------------------------------|
| Хранение сообщений    | На устройстве клиента | На сервере                    |
| Синхронизация         | Не поддерживается     | Поддерживается                |
| Работа с папками      | Не поддерживается     | Возможность организации писем |
| Резервное копирование | Зависит от клиента    | Письма сохраняются на сервере |
| Подходит для          | Одного устройства     | Нескольких устройств          |
| Порты по умолчанию    | 110 (995 для SSL/TLS) | 143 (993 для SSL/TLS)         |

---

## **Заключение**

- **POP** удобен для пользователей, которые работают только с одним устройством и хотят хранить письма локально.
- **IMAP** лучше подходит для пользователей, работающих с почтой с нескольких устройств, так как обеспечивает
  синхронизацию и хранение сообщений на сервере.
- Выбор протокола зависит от требований к доступности, синхронизации и резервному копированию электронной почты.
...
Одной из причин использования технологии NAT является дефицит _IPv4-адресов_. Если
...
IPv4-адресов, то оно может прибегнуть к технологии NAT. В этом случае для адресации
внутренних узлов служат специально зарезервированные для этих целей **частные адреса.**

Технология NAT также оказывается полезной, когда предприятие из соображений безопасности желает скрыть адреса узлов
своей сети, чтобы не дать возможности злоумышленникам составить представление о структуре и масштабах корпоративной
сети, а также
о структуре и интенсивности исходящего и входящего трафиков.

> Именно технология NAT стояла у истоков зарождения файерволов как отдельного класса продуктов. В начале 90-х годов,
> когда дефицит адресов IPv4 еще мало ощущался, несколько специалистов основали компанию Network Translation и
> разработали
> программный продукт PIX, который
> позволял транслировать сетевые адреса. Позднее эту компанию приобрела компания Cisco,
> а программный продукт стал знаменитым фаерйволом Cisco PIX Firewall, являющимся одним из
...
из которых — **традиционная технология трансляции сетевых адресов** — позволяет узлам из частной сети прозрачным для
пользователей образом получать доступ
к узлам внешних сетей. Подчеркнем, что в данном варианте NAT решается проблема организации только тех сеансов связи,
которые исходят из частной сети.

Идея технологии NAT — в следующем. Пусть сеть предприятия образует тупиковый домен,
узлам которого присвоены частные адреса (рис. 28.5). На маршрутизаторе, связывающем
сеть предприятия с внешней сетью, установлено программное обеспечение NAT. NAT-устройство динамически отображает набор
частных адресов {IP*} на набор глобальных
адресов {IP}, полученных предприятием от поставщика услуг и присвоенных внешнему
интерфейсу маршрутизатора предприятия.

![Схема традиционной технологии NAT](img/img_18.png)

Важным для работы NAT-устройства является правило распространения маршрутных
...
о них отфильтровываются при передаче на внешние интерфейсы. Поэтому внутренние
маршрутизаторы «знают» маршруты ко всем внешним сетям, а внешним маршрутизаторам
ничего не известно о существовании частных сетей. Традиционная технология NAT подразделяется на технологии `базовой
трансляции сетевых адресов (Basic Network Address
Translation, Basic NAT)` и `трансляции сетевых адресов и портов (Network Address Port
Translation, NAPT)`. В технологии Basic NAT для отображения используются только IP-
адреса, а в технологии NAPT — еще и так называемые транспортные идентификаторы,
в качестве которых чаще всего выступают порты TCP и UDP.
...
Файервол защищает внутреннюю сеть (например, локальную сеть предприятия
...
_вспомогательные функции_ защиты, в частности:

* антивирусная защита;
* шифрование трафика;
* логическое посредничество между внутренними клиентами и внешними серверами
  (функция прокси-сервера);
* трансляция сетевых адресов (NAT);
* фильтрация сообщений по содержимому, включая типы передаваемых файлов, имена
  DNS и ключевые слова;
* предупреждение и обнаружение вторжений и сетевых атак;
* функции VPN.

Более полное опредление фаервола

> `Файервол` — это программно-аппаратный комплекс, выполняющий разнообразные функции по
> защите внутренней сети, набор которых может меняться в зависимости от типа, модели и конкретной конфигурации
> файервола, при этом минимальный набор функций должен включать фильтрацию трафика для предотвращения сетевых атак и
> аудит событий, связанных с фильтрацией.
> Файерволы осуществляют экранирование защищаемого объекта и формируют его внешнее представление. В сетевой среде
> файерволы часто являются первым и самым мощным
> рубежом обороны.
...
* `программный файервол` реализован как программная система, работающая под управлением универсальной ОС, такой как
  Microsoft Windows, Linux или Mac OS (возможно,
  имеющая версии для нескольких универсальных ОС);
* `аппаратный файервол` реализован как набор дополнительных функций маршрутизатора, относящихся к фильтрации (реже —
  Ethernet-коммутатора);
* `программно-аппаратный файервол` представляет собой программную систему, работающую на базе специализированной
  платформы. Обычно в качестве такой специализированной платформы выступает аппаратный сервер, сертифицированный для
  работы
  с программным обеспечением файервола, плюс установленная на нем универсальная
  ОС с набором специфических настроек, обеспечивающих максимальный уровень безопасности.
  В зависимости от способа фильтрации различаются файерволы без запоминания состояния
  и файерволы с запоминанием состояния:
* `файерволы без запоминания состояния (stateless)` выполняют фильтрацию на основе
  статических правил, при этом не отслеживаются состояния соединений (сеансов);
* `файерволы с запоминанием состояния (stateful)` принимают решения динамически,
...
с возможностью фильтрации кадров канального уровня на основе задаваемых администратором списков доступа, могут условно
быть отнесены к `файерволам канального
уровня.`

`Файерволы сетевого уровня`, называемые также `файерволами с фильтрацией пакетов
(packet filtering firewall)`, в полном соответствии со своим названием решают задачу фильтрации пакетов по IP-адресам (
как источника, так и приемника), а также по значению поля
протокола верхнего уровня — в пакет сетевого уровня могут быть вложены сообщения
протоколов TCP, UDP, ICMP и др. Более того, несмотря на свое название, такие файерволы работают и на более высоком,
транспортном уровне, то есть на уровне портов TCP
и UDP, но только на основе статических правил, при которых не отслеживаются состояния
соединений, то есть в режиме без запоминания состояния.

`Файерволы сеансового уровня` являются файерволами с запоминанием состояний соединений на уровнях ниже прикладного.
Файерволы сеансового уровня эффективно
противодействуют тем видам атак на протокол TCP, в которых нарушается трехшаговая
процедура установления соединения.

`Файерволы прикладного уровня` способны интерпретировать, анализировать и контролировать содержимое сообщений, которыми
обмениваются приложения. Они также работают
...
как протоколы SSH, HTTP, FTP, SQL, SMTP, POP3, IMAP, FTP, SSH, SQL и др.

За счет дополнительных модулей файерволы прикладного уровня
могут поддерживать самые разные функции защиты программного обеспечения, например:

* _трансляция внутренних IP-адресов_ пользователей на основе стандарта NAT;
* _антивирусный контроль_ загружаемых пользователем файлов и получаемых писем «на
  лету»;
* _контроль контента_, заключающийся, например, в ограничении доступа пользователей
...
* _транзитная аутентификация пользователей_, обращающихся к некоторому приложению на внутреннем сервере, — эта функция
  полезна для тех приложений, которые либо
  не выполняют аутентификацию пользователей совсем, либо делают это незащищенным
  способом, как, например, FTP-сервер, который принимает пароли пользователей в открытом виде: файервол перехватывает
  обращение пользователя к FTP-серверу (команду USER) и организует сеанс логического входа пользователя, например, с
  сервером
  аутентификации Kerberos, если именно такой способ аутентификации применяется
  в корпоративной сети;
* _централизованное шифрование электронных писем пользователей_, что избавляет пользователей от необходимости
  конфигурировать такую функцию на своих клиентских
  компьютерах (для этого файервол должен хранить цифровые сертификаты пользователей);
* _функции шлюза VPN_ с удаленными подразделениями предприятия и удаленными
  пользователями.

# Файервол: настройка и фильтрация пакетов

## **Настройка фильтрации пакетов**

Фильтрация пакетов в файерволе основывается на правилах, которые задаются администратором. Эти правила определяют, как
файервол обрабатывает входящий, исходящий и пересылаемый трафик.

### **Основные элементы настройки**

1. **Критерии фильтрации**:
    - **IP-адреса**:
        - Фильтрация по IP-адресу источника или получателя.
        - Пример: блокировка запросов с IP-адреса `192.168.1.10`.
    - **Порты**:
        - Разрешение или запрет трафика на определенных портах.
        - Пример: блокировка порта 23 (Telnet) для повышения безопасности.
    - **Протоколы**:
        - Фильтрация пакетов TCP, UDP, ICMP и других.
        - Пример: разрешение только TCP-трафика для HTTP (порт 80).
    - **Флаги TCP**:
        - Контроль за состоянием соединения.
        - Пример: разрешение пакетов с флагом SYN только для нового соединения.

2. **Правила фильтрации**:
    - **Пропуск (Allow)**:
        - Трафик, соответствующий правилам, разрешается.
    - **Блокировка (Deny)**:
        - Трафик, соответствующий правилам, блокируется.
    - **Перенаправление (Redirect)**:
        - Трафик направляется на другой узел или порт для обработки.

3. **Порядок правил**:
    - Файервол обрабатывает правила сверху вниз.
    - Первое совпадение с правилом определяет действие, выполняемое над пакетом.

4. **Логи и аудит**:
    - Настройка регистрации событий для мониторинга заблокированного или разрешенного трафика.

---

## **Примеры настройки фильтрации**

### **1. Простое правило блокировки**:

- **Сценарий**:
    - Блокировать весь входящий трафик с IP-адреса `192.168.1.10` на порт 22 (SSH).
- **Правило**:

Deny IP 192.168.1.10:22

- **Результат**:
- Любой трафик от указанного источника на порт 22 будет отклонен.

### **2. Разрешение доступа к веб-серверам**:

- **Сценарий**:
- Разрешить доступ к портам 80 (HTTP) и 443 (HTTPS) только из подсети `10.0.0.0/24`.
- **Правило**:

Allow IP 10.0.0.0/24:80
Allow IP 10.0.0.0/24:443

- **Результат**:
- Трафик на веб-сервер будет разрешен только из указанной подсети.

### **3. Фильтрация по протоколу ICMP**:

- **Сценарий**:
- Блокировать ICMP-запросы (ping) для повышения безопасности.
- **Правило**:

Deny ICMP Any Any

- **Результат**:
- Любые ICMP-пакеты будут блокированы.

### **4. Ограничение времени доступа**:

- **Сценарий**:
- Разрешить доступ на порт 25 (SMTP) только в рабочее время (9:00–18:00).
- **Правило**:

Allow IP Any:25 Time 09:00-18:00
Deny IP Any:25

- **Результат**:
- SMTP-соединения будут доступны только в указанные часы.

---

## **Эффективная настройка фильтрации**

1. **Минимизация разрешений**:

- Настройка только необходимых правил для минимизации рисков.

2. **Приоритет правил**:

- Расположить наиболее специфичные правила выше общих.

3. **Мониторинг и аудит**:

- Включить логирование для анализа событий и выявления подозрительного трафика.

4. **Регулярное обновление**:

- Обновлять правила в зависимости от новых угроз и изменений в сети.

Фильтрация пакетов — это ключевая функция файерволов, позволяющая эффективно управлять доступом, защищать сеть и
минимизировать риски. Правильная настройка правил играет решающую роль в обеспечении безопасности.
